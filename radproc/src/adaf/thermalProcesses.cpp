						...
						Vector& energies, const int flags[], Matrix& lumOut)
{
	...
	for (int jE=0;jE<nE;jE++) {
		...
		st.photon.ps.iterate([&](const SpaceIterator& itER) {
			...
			double kappa = (b_nu > 0.0) ? (xSy+xBr)/(4.0*pi*b_nu) : 0.0;
			...
			if (!std::isfinite(fluxSy)) fluxSy = 0.0;
			if (!std::isfinite(fluxBr)) fluxBr = 0.0;
						...
						Matrix lumOut, Matrix lumOutCD, Matrix lumOutRefl, const string& filename)
{
	...
	for (size_t jE=0;jE<nE;jE++) {
		...
		for (size_t jR=0;jR<nR;jR++) {
			...
			double r = st.denf_i.ps[DIM_R][jR]/(schwRadius/2);
	...
	// Output SED integrated up to specific radii: r < R_i / cos(alpha)
	{
		const double alpha = 17.0 * M_PI / 180.0;
		const double cosAlpha = cos(alpha);
		const double radii_Rg[] = {7.3, 26.0, 65.0, 195.0, 650.0, 3.5e5};
		const size_t nRadii = 6;
		
		ofstream fileRadii;
		fileRadii.open("lumThermal_radii.dat", ios::out);
		for (size_t jE = 0; jE < nE; jE++) {
			double E = energies[jE];
			double frequency = E / planck;
			double lumTotRadii[6] = {};
			for (size_t jR = 0; jR < nR; jR++) {
				double r_Rg = st.denf_i.ps[DIM_R][jR] / gravRadius;
				Vector lumOutVec(nE, 0.0);
				for (size_t jjE = 0; jjE < nE; jjE++)
					lumOutVec[jjE] = lumOut[jjE][jR];
				double localEnergy = energies[jE] / redshift_to_inf[jR];
				double contrib = lumInterp(lumOutVec, energies, jE, nE, localEnergy)
								* pow(redshift_to_inf[jR], 3) * escapeAi[jR];
				for (size_t k = 0; k < nRadii; k++) {
					if (r_Rg < radii_Rg[k] / cosAlpha)
						lumTotRadii[k] += contrib;
				}
			}
			fileRadii << setw(10) << setiosflags(ios::fixed) << scientific << setprecision(2) << frequency;
			for (size_t k = 0; k < nRadii; k++)
				fileRadii << setw(10) << setiosflags(ios::fixed) << scientific << setprecision(2) << lumTotRadii[k] * frequency;
			fileRadii << endl;
		}
		fileRadii.close();
	}
	
	// Output synchrotron I_nu at each radius for specific frequencies
	{
		const double targetFreqs[] = {22.0e9, 43.0e9, 86.0e9};
		const size_t nFreqs = 3;
		
		ofstream fileSyRadii;
		fileSyRadii.open("lumSy_radii.dat", ios::out);
		size_t jR = 0;
		st.photon.ps.iterate([&](const SpaceIterator& itR) {
			double r = itR.val(DIM_R);
			double r_Rg = r / gravRadius;
			double temp_e = st.tempElectrons.get(itR);
			double magf = st.magf.get(itR);
			double dens_e = st.denf_e.get(itR);
			double h = height_fun(r);
			
			fileSyRadii << setw(10) << setiosflags(ios::fixed) << scientific << setprecision(2) << r_Rg;
			for (size_t k = 0; k < nFreqs; k++) {
				double freq = targetFreqs[k];
				double energy = freq * planck;
				double jSy = jSync(energy, temp_e, magf, dens_e);
				double b_nu = bb(freq, temp_e);
				double alpha_nu = (b_nu > 0.0) ? jSy / b_nu : 0.0;
				double tau_star = 0.5 * sqrt(M_PI) * alpha_nu * h;
				double I_nu = 0.5 * b_nu * (1.0 - exp(-2.0 * sqrt(3.0) * tau_star));
				fileSyRadii << setw(14) << setiosflags(ios::fixed) << scientific << setprecision(4)
							<< I_nu;
			}
			fileSyRadii << endl;
			jR++;
