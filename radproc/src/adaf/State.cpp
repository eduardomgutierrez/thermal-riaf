...
std::function<double(double)> l10 = [](double x) { return (x > 0.0) ? log10(x) : -300.0;};
std::function<double(double)> e10 = [](double x) { return pow(10.0, x); };
...
void State::initializeParticle(Particle& p,boost::property_tree::ptree& cfg)
{
	...
	// add dimension for r
	double innerRadius, edgeRadius;
	if (readPrecomputedADAF && !precomp_r.empty()) {
		// Use range from precomputed sol_spin file (r is in r_g units, convert to cm)
		// precomp_r is ordered from large to small, so back() is inner, front() is outer
		// Enforce innerRadius >= horizonRadius for physical validity
		double precomp_rIn = precomp_r.back() * gravRadius;  // Convert r/r_g to cm
		innerRadius = std::max(precomp_rIn, horizonRadius);
		edgeRadius = precomp_r.front() * gravRadius;  // Convert r/r_g to cm
	} else {
		innerRadius = exp(logr.front())*schwRadius;
		edgeRadius = exp(logr.back())*schwRadius;
	}
