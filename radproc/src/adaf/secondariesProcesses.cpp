...
void secondariesRadiationProcesses(State& st, const std::string& filename)
{
	...
	for (int E_ix=0;E_ix<nEnt;E_ix++) {
		...
		st.ntPhoton.ps.iterate([&](const SpaceIterator& iR) {
			...
			double tau = (E/EV_TO_ERG < 1e3 ? 0.5*sqrt(pi)*alpha_tot*height : 0.0);
            ...
            double totLocal = 0.0;
            if (E/EV_TO_ERG < 1e4) {
                totLocal = (tau > 1.0e-10) ? 
										factor*vol*eTot/alpha_tot * 
                                        (1.0-exp(-2.0*sqrt(3.0)*tau)) : 
											eTot*vol;
            } else {
                totLocal = (taugg > 1.0e-10) ? 
										factor*vol*eTot/kappagg * 
                                        (1.0-exp(-2.0*sqrt(3.0)*taugg)) : 
											eTot*vol;
            }
			//totLocal = eTot*vol*exp(-taugg);
			
            totNotAbs[E_ix][jR] = totLocal;
			totAbs[E_ix][jR] = eTot * vol;
	...
	double lum_eSy, lum_muSy, lum_piSy, lum_eIC, lum_piPP, lum_piPG, lum_eeAnn, lum_totNotAbs, lum_totAbs;
	for (size_t jE=0;jE<nEnt;jE++) {
		lum_eSy = lum_muSy = lum_piSy = lum_eIC = lum_piPP = lum_piPG = lum_eeAnn = lum_totNotAbs = lum_totAbs = 0.0;
		...
		for (size_t jR=0;jR<nR;jR++) {
			Vector eSyVec(nEnt,0.0), muSyVec(nEnt,0.0), piSyVec(nEnt,0.0), eICVec(nEnt,0.0), piPPVec(nEnt,0.0),
					piPGVec(nEnt,0.0), eeAnnVec(nEnt,0.0), totNotAbsVec(nEnt,0.0), totAbsVec(nEnt,0.0);
...
void secondariesProcesses(State& st)
{
	...
	*/
	...
	writeEandRParamSpace("muonInjection",st.ntMuon.injection,0,1);
    ...
    double sumPair = 0.0;
    double sumProton = 0.0;
    st.ntPhoton.ps.iterate([&](const SpaceIterator& iR) {
        double r = iR.val(DIM_R);
        sumPair += integSimpsonLog(st.ntPair.emin(), st.ntPair.emax(), [&st,&iR] (double e)
					{
						double inj = pairBH(e, st.ntProton, st.photon.distribution, iR.coord,
								st.photon.emin(), st.photon.emax());
						return e*inj;
					}, 100) * volume(r);
        sumProton += integSimpsonLog(st.ntProton.emin(), st.ntProton.emax(), [&st,&iR] (double e)
					{
						double dist = st.ntProton.distribution.interpolate({{DIM_E, e}}, &iR.coord);
						return dist*lossesPhotoPair_2(e, st.ntProton, st.photon.distribution, iR.coord, st.photon.emin(),
                        st.photon.emax());
					}, 100) * volume(r);
    },{0,-1,0});
    cout << "pair = " << sumPair << endl;
    cout << "proton = " << sumProton << endl;
}