...
/*******************************************************************************
 * probInterpolated() - Method 1 interpolation
 * 
 * Performs trilinear interpolation in the 3D probability table P(γ, ν', ν)
 * for a single electron Lorentz factor γ.
 * 
 * Used by: probTemp() which then thermally averages over γ
 * Table source: comptonProbMatrix.dat (generated by comptonMatrix())
 ******************************************************************************/
...
/*******************************************************************************
 * probTemp() - Method 1 thermal averaging
 * 
 * Thermally averages probInterpolated() over a Maxwell-Jüttner distribution
 * using Gauss-Laguerre quadrature. This is called AT RUNTIME for method 1.
 * 
 * Input:
 *   comp     - Precomputed P(γ,ν',ν) table from comptonProbMatrix.dat
 *   nu       - Output photon frequency [Hz]
 *   nuPrim   - Input photon frequency [Hz]
 *   normtemp - Normalized temperature: kT_e / (m_e c^2)
 * 
 * Returns: Thermally-averaged scattering probability P(ν,ν',Te)
 ******************************************************************************/
...
/*******************************************************************************
 * probInterpolated2() - Method 0 interpolation
 * 
 * Performs trilinear interpolation in the 3D probability table P(Te, ν', ν)
 * where Te is electron temperature. The thermal averaging was already done
 * when generating the table.
 * 
 * Used by: comptonNewNew() when comptonMethod=0
 * Table source: comptonProbMatrix2.dat (generated by comptonMatrix2())
 * 
 * This is FASTER at runtime than method 1 because no thermal integration needed.
 ******************************************************************************/
...
/*******************************************************************************
 * comptonMatrix() - Table generator for Method 1 (γ-based tables)
 * 
 * Generates the 3D scattering probability table P(ν,ν',γ) for MONOENERGETIC
 * electrons with Lorentz factor γ. Thermal averaging is done at runtime.
 * 
 * Output files:
 *   - comptonProbMatrix.dat : The P(ν,ν',γ) table
 *   - comptonProbTot.dat    : Debug file with kinematic limits
 * 
 * Uses exact Klein-Nishina cross section via probexactNew().
 * Called when calculateRedMatrix=1 AND comptonMethod=1
 ******************************************************************************/
void comptonMatrix()
{
...
/*******************************************************************************
 * comptonMatrix2() - Table generator for Method 0 (Te-based tables)
 * 
 * Generates the 3D scattering probability table P(ν,ν',Te) where the thermal
 * averaging over the Maxwell-Jüttner distribution is done DURING TABLE GENERATION.
 * This makes runtime much faster (just interpolation, no integration).
 * 
 * Output files:
 *   - comptonProbMatrix2.dat : The P(ν,ν',Te) table (thermally averaged)
 *   - tempComptonVec.dat     : Temperature grid points
 *   - nuPrimComptonVec.dat   : Input frequency grid points
 *   - nuComptonVec.dat       : Output frequency grid points
 * 
 * Uses Gauss-Laguerre quadrature for thermal averaging.
 * Called when calculateRedMatrix=1 AND comptonMethod=0
 * 
 * RECOMMENDED: This is the preferred method as it allows changing frequency
 * bounds via nuMinCompton/nuMaxCompton in parameters.json
 ******************************************************************************/
void comptonMatrix2()
{
	double pasoTemp = pow(tempMaxCompton/tempMinCompton,1.0/nTempCompton);
...
/**
 * @brief Compute scattered luminosity at frequency nu using precomputed tables.
 *
 * For comptonMethod=0: Uses probInterpolated2 with Te-based tables (.dat files).
 * For comptonMethod=1: Uses probTemp with gamma-based tables (.bin files).
 *
 * Integrates: L_out(ν) = Σ_ν' L_in(ν') × P(ν'→ν,Te) × (ν/ν') × dν'
 *
 * @param tempVec Temperature grid from table (comptonMethod=0)
 * @param nuPrimVec Input frequency grid (comptonMethod=0)
 * @param nuVec Output frequency grid (comptonMethod=0)
 * @param probVec Flattened probability table
 * @param lumIn Input luminosity spectrum L_ν [erg/s/Hz]
 * @param normtemp Normalized temperature kT_e/(m_e c^2)
 * @param nu Output frequency [Hz]
 * @param energies Energy grid [erg]
 * @param jE Energy index (used for reference)
 * @return Scattered luminosity at frequency nu [erg/s/Hz]
 */
...
/**
 * @brief Fill probability matrix row using precomputed tables.
 *
 * For a given temperature index jTemp and output energy jE, fills the
 * probability matrix p[jTemp, jE', jE] for all input energies jE'.
 * Used for comptonMethod=0 (Te-based) or comptonMethod=1 (\u03b3-based).
 *
 * @param tempVec Temperature grid from table (method 0)
 * @param nuPrimVec Input frequency grid (method 0)
 * @param nuVec Output frequency grid (method 0)
 * @param probVec Flattened probability table
 * @param jTemp Temperature index for matrix storage
 * @param energies Energy grid
 * @param jE Output energy index
 * @param p Output probability matrix [nTemp \u00d7 nE \u00d7 nE]
 * @param normtemp Normalized temperature kT_e/(m_e c^2)
 */
...
/**
 * @brief Load binary probability tables and fill probability matrix p.
 *
 * This is for comptonMethod=1. Reads binary files:
 *   - probs.bin: P(Te, ν', ν) probability values
 *   - nt.bin: log10(kT_e/m_e c^2) temperature grid
 *   - om.bin: log10(hν_out/m_e c^2) output frequency grid
 *   - omp.bin: log10(hν_in/m_e c^2) input frequency grid
 *
 * Performs trilinear interpolation in (Te, ν', ν) space to fill the
 * probability matrix p[jR,jE',jE] for all radii and energy pairs.
 *
 * Note: These binary tables have FIXED frequency ranges determined at
 * generation time and cannot be regenerated by this code.
 *
 * @param st State containing photon grid and electron temperatures
 * @param p Output probability matrix [nR × nE × nE]
 * @param redshift_to_inf Redshift factors for each radius
 */
...
/**
 * @brief Fill probability matrix row using on-the-fly exact computation.
 *
 * For a given output energy jE, computes P(ν'→ν|Te) for all input
 * energies using exact Klein-Nishina cross section with thermal averaging.
 * Used to populate probability matrix for comptonMethod=3.
 *
 * @param jR Radius index for matrix storage
 * @param energies Energy grid
 * @param jE Output energy index
 * @param p Output probability matrix to fill [nR × nE × nE]
 * @param normtemp Normalized temperature kT_e/(m_e c^2)
 */
...
{
	// Safety check for NaN or invalid input
	if (!std::isfinite(redE) || redE <= 0.0) return 0.0;

	if (redE > energies[nE-1]) return lum[nE-1];
	if (redE < energies[0]) return lum[0];
	
	size_t j = jE;
	// Bounds-checked search for interpolation index
	if (redE > energies[j]) {
		while (j < nE-1 && redE > energies[j]) j++;
	}
	if (redE < energies[j]) {
		while (j > 0 && redE < energies[j]) j--;
		j++;
	}
	// Final bounds check
	if (j == 0) j = 1;
...
/**
 * @brief Apply precomputed probability matrix to get scattered luminosity.
 *
 * Uses probability matrix p[jTemp,jE',jE] already filled by cNew() or
 * comptonNewNewPruebaVector() to compute scattered luminosity.
 *
 * L_out(jE) = Σ_{jE'} L_in(jE') × p[jTemp,jE',jE] × (E_out/E_in) × dν'
 *
 * @param p Probability matrix [nTemp × nE × nE]
 * @param lumIn Input luminosity spectrum
 * @param jTemp Temperature index in probability matrix
 * @param energies Energy grid
 * @param jE Output energy index
 * @return Scattered luminosity at energy jE
 */
...
/**
 * @brief On-the-fly Compton scattering computation (comptonMethod=3).
 *
 * Computes scattered luminosity without precomputed tables by evaluating
 * the exact Klein-Nishina cross section and performing thermal averaging
 * using Gauss-Laguerre quadrature.
 *
 * For each input photon ν' and output photon ν:
 *   1. Sample electron Lorentz factors γ from Maxwell-Jüttner distribution
 *   2. Compute kinematic limits (ω_min, ω_max) for the scattering
 *   3. Evaluate exact probability P(ν'→ν|γ) using probexactNew()
 *   4. Weight by Maxwell-Jüttner and sum
 *
 * Most flexible but SLOWEST method - no precomputed tables required.
 *
 * @param lumIn Input luminosity spectrum [erg/s/Hz]
 * @param normtemp Normalized temperature kT_e/(m_e c^2)
 * @param nu Output frequency [Hz]
 * @param energies Energy grid [erg]
 * @param jE Output energy index
...
/**
 * @brief Local iterative Compton computation for one scattering.
 *
 * Used in iterative Comptonization scheme (comptonMethod=2) where
 * multiple scatterings are computed iteratively. This function computes
 * one scattering event.
 *
 * Performs thermal averaging over electron distribution using
 * Gauss-Laguerre quadrature, with scattering rates from rate()
 * and probabilities from probInterpolated().
 *
 * @param comptonVec Probability table for gamma-based interpolation
 * @param lum Input photon luminosity spectrum
 * @param normtemp Normalized temperature kT_e/(m_e c^2)
 * @param jE Output energy index
 * @param energies Energy grid [erg]
 * @param nE Number of energy bins
 * @return Scattered luminosity contribution [erg/s/Hz]
 */
...
/**
 * @brief Alternative local Compton computation with photon number density.
 *
 * Similar to comptonNewLocal but uses photon number density n_ph(ν)
 * instead of luminosity. Includes stimulated emission factor:
 *   (1 + 2π n_ph(ν_out) c²/ν²)
 *
 * Uses approximate mapping: ν' ≈ ν/(4γ²/3) for dominant scattering.
 *
 * @param nPh Photon number density spectrum
 * @param normtemp Normalized temperature kT_e/(m_e c^2)
 * @param jE Output energy index
 * @param energies Energy grid [erg]
 * @return Scattered photon contribution
 */
