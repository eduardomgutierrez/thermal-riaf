...
int calculateNonThermalHE;              // If 1 calculate non-thermal high-energy processes.
int calculateSecondaries;				// If 1 calculate electromagnetic cascades.
int calculateNeutrinos;					// If 1 calculate neutrino injection.
...
// Precomputed ADAF from sol_spin file (alternative reading mode)
int readPrecomputedADAF;				// If 1 read ADAF properties from sol_spin file.
double R_high;							// R_high parameter for Te = Ti/R prescription.
Vector precomp_r;						// Precomputed radii (dimensionless).
Vector precomp_density;					// Precomputed mass density [g cm^-3].
Vector precomp_v;						// Precomputed velocity / c.
Vector precomp_vphi;					// Precomputed azimuthal profile (v_phi/c or Omega [1/s]).
Vector precomp_B;						// Precomputed magnetic field [Gauss].
Vector precomp_HR;						// Precomputed H/R.
Vector precomp_beta;					// Precomputed plasma beta.
Vector precomp_Temp;					// Precomputed temperature [K].
int precompPhiIsOmega;					// Deprecated: precomp_vphi is read from v_phi/c column.
...
double gravRadius;						// Gravitational Radius r_g = GM/c^2 = R_S/2 [cm]
double blackHoleSpin;					// Black hole spin parameter a (dimensionless, 0 <= a < 1)
double horizonRadius;					// Event horizon radius [cm] (depends on spin)
