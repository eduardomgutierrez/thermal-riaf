...
void injectionPair(Particle& p, State& st, int it)
{
	...
	for (size_t jR=0;jR<nR;jR++) {
		...
		p.ps.iterate([&](const SpaceIterator& iRE) {
			...
			double injectionGammaGamma = (Eee > 10*electronRestEnergy) ?
                    pairGammaGammaNew(Eee, st.ntPhoton.distribution, st.photon.distribution,
                        iRE, st.photon.emin(), st.photon.emax(), st.ntPhoton.emin(), st.ntPhoton.emax()) : 0.;
			
			//double tcool = Eee/lossesSyn(Eee,st.magf.get(iR),p);
			p.injection.set(iRE, p.injection.get(iRE) + injectionGammaGamma);
			secondariesGammaGammaFile << safeLog10(Eee/EV_TO_ERG) << "\t" << iRE.coord[DIM_R] << "\t"
								<< r/schwRadius << "\t" << injectionGammaGamma * vol << endl;
		},{-1,jR,0});
...
{
    double sumTot = 0.0;
	show_message(msgStart,Module_neutrinoInjection);
