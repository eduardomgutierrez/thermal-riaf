...
double jBremss(double energy, double temp, double dens_i, double dens_e) {
    ...
    // Safety checks
    if (temp <= 0.0 || energy <= 0.0 || dens_i <= 0.0 || dens_e <= 0.0) {
        return 0.0;
    }
    if (!std::isfinite(temp) || !std::isfinite(energy)) {
        return 0.0;
    }
    ...
    // Avoid underflow/overflow in exp
    if (temp_aux <= 0.0 || 1.0/temp_aux > 700.0) return 0.0;
    
    double result = aux / (boltzmann*temp) * planck * exp(- (1.0/temp_aux)) * gauntFactor(temp_aux);
	return std::isfinite(result) ? result / (4.0*pi) : 0.0;
}