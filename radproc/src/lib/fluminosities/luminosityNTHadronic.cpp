...
double fntHadron(double x, const Particle& p, const SpaceCoord& psc) //funcion a integrar   x=Ecreator; L=L(Ega)
{	
	double Kpi = 0.17;
	...
	double l = log10(eval/1.602); //evaluada en eval
	double sigma = 1.e-27 * (34.3+1.88*l+0.25*l*l) * P2(1.0 - pow(Eth/eval,4));
	double pionEmiss = sigma*distCreator/Kpi;  //sigma = crossSectionHadronicDelta(Ekin)
															  //lo saco asi pongo la condicion Ekin > Ethr en el limite de la int
	
	double result = pionEmiss / sqrt( (x-neutralPionMass*cLight2) * (x + neutralPionMass*cLight2) );
	return (x > 1.01*neutralPionMass*cLight2 ? result : 0.0);
}
...
double Fg(double x, double Ep)
{
    double L = log(Ep/1.602);
    double Bg = 1.3 + L * (0.14 + 0.011*L);
    double betag = 1.0 / (1.79 + L * (0.11 + 0.008*L));
    double kg = 1.0 / (0.801 + L * (0.049 + 0.014*L));
    double x_to_betag = pow(x, betag);
    double factor1 = pow((1.0 - x_to_betag)/(1.0 + kg*x_to_betag*(1.0-x_to_betag)), 4);
    double aux1 = 4.0*betag*x_to_betag;
    double factor2 = 1.0/log(x) - aux1/(1.0-x_to_betag) -
                        aux1*kg*(1.0-2.0*x_to_betag)/(1.0 + kg*x_to_betag*(1.0-x_to_betag));
    return (x < 1.0 ? Bg * log(x)/x * factor1 * factor2 : 0.0);
}

double luminosityNTHadronic2(double Eg, const Particle& creator,
	const double density, const SpaceCoord& psc)
{
    double inf = std::max(Eg, 0.1602);
    double sup = creator.emax();
    double integ = integSimpsonLog(inf, sup,
                        [&creator, &psc, Eg]
                        (double Ep)
                        {
                            double Np = creator.distribution.interpolate({{0, Ep}}, &psc);
                            double sigmapp = crossSectionHadronic(Ep);
                            return Np/Ep * sigmapp * Fg(Eg/Ep, Ep);
                        }, 100);
	return cLight * density * integ * Eg*Eg;
}
...
double luminosityNTHadronic(double Eg, const Particle& creator,
                            const double density, const SpaceCoord& psc)
{
	double Kpi = 0.17;
	double thr = 0.0016; //1GeV
	double Max  = creator.emax();   //esto es un infinito 
	double Min  = std::max(Eg + P2(chargedPionMass*cLight2)/(4*Eg), thr*Kpi); //== Ekin > Ethr
	Min = Eg + 0.25*P2(neutralPionMass*cLight2)/Eg;
	double integral = integSimpsonLog(Min, Max,
                [&]
                (double Ep)
				{
					return fntHadron(Ep, creator, psc);
				}, 100);
	//double integral = RungeKuttaSimple(Min, Max, 
	//	[&](double x) {return fntHadron(x, creator, density, psc); }
	//);    //integra entre Emin y Emax
	return 2.0*density*cLight*integral*P2(Eg); // [erg s^-1 cm^-3 ]
}


double luminosityNTHadronicMatching(double Eg, const Particle& creator,
	const double density, const SpaceCoord& psc)
{
	double Kpi = 0.17;
	double thr = 0.001602; //1GeV
    double inf1  = std::max(Eg + 0.25*P2(neutralPionMass*cLight2)/Eg, thr*Kpi);
	double sup1  = 0.1602;
	double integ1 = 2.0 * integSimpsonLog(inf1, sup1,
                [&]
                (double Ep)
				{
					return fntHadron(Ep, creator, psc);
				}, 30);
    double inf2 = std::max(Eg, sup1);
    double sup2 = creator.emax();
    double integ2 = integSimpsonLog(inf2, sup2,
                        [&creator, &psc, Eg]
                        (double Ep)
                        {
                            double Np = creator.distribution.interpolate({{0, Ep}}, &psc);
	...
	//	[&](double x) {return fntHadron(x, creator, density, psc); }
	//);    //integra entre Emin y Emax
	return cLight * density * (integ1 + integ2) * P2(Eg); // [erg s^-1 cm^-3 ]
}
/*
