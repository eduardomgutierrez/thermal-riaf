...
{
    double gammap = e / (protonMass*cLight2);
	double inf = pairThresholdPH/(2.0*gammap);
    inf = std::max(inf, phEmin);
	double integ = integSimpsonLog(inf, phEmax, [gammap, &c, &tpf, &distCoord] (double Eph)
		{
			double nPh = tpf.interpolate({{0, Eph}}, &distCoord);
			double inf2 = pairThresholdPH;
			double sup2 = 2.0*Eph*gammap;
			double integ2 = integSimpsonLog(inf2, sup2, [&](double ep)
						{
							return ep*crossSectionBetheHeitler2(ep);
						}, 80);
			return nPh/(Eph*Eph) * integ2;
		}, 80);
	//return 0.5*P2(protonMass)*gsl_pow_5(cLight)/(e*e) * integ;
    return 0.5 * cLight / P2(gammap) * integ;
}

double rateBH2(double e, const Particle& c, const ParamSpaceValues& tpf, const SpaceCoord& distCoord,
					double phEmin, double phEmax)
{
    double gammap = e / (protonMass*cLight2);
	double inf = pairThresholdPH/(2.0*gammap);
	...
	double integ = integSimpsonLog(inf, phEmax, [gammap, &c, &tpf, &distCoord] (double Eph)
		{
			...
			double integ2 = integSimpsonLog(inf2, sup2, [&](double ep)
						{
							return ep*crossSectionBetheHeitler2(ep)*inelasticityBetheHeitler(ep);
						}, 80);
			return nPh/(Eph*Eph) * integ2;
		}, 80);
	//return 0.5*P2(protonMass)*gsl_pow_5(cLight)/(e*e) * integ;
...
double pairBH(double Eee, const Particle& creator, const ParamSpaceValues& tpf, const SpaceCoord& distCoord, double tpEmin, double tpEmax)
{
    ...
    double rateBH = rateBH2(evalE, creator, tpf, distCoord, tpEmin, tpEmax);
	return P2(protonMass/electronMass) * rateBH * protonDist;
}
