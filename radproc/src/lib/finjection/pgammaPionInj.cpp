					...
					double tpEmin, double tpEmax)
{
	...
	return 0.25*cLight/(g*g) * integSimpsonLog(inf,tpEmax,[g,&tpf,&distCoord](double Eph)
				{
					double nPh = tpf.interpolate({{0,Eph}},&distCoord);
					return nPh/(Eph*Eph) * integKSigmaPG(Eph,g);
				},100);
	
}

//double pgammaPionInj(double E, Vector Nproton, Particle& particle, Particle& proton, fun1 tpf)  
double pgammaPionInj(double Epi, const Particle& creator,	const ParamSpaceValues& tpf, 
						const SpaceCoord& distCoord, double tpEmin, double tpEmax)
{
	double cincoEpi = 5.0*Epi;
	double protonDist = (cincoEpi > creator.emin() && cincoEpi < creator.emax()) ? 
					creator.distribution.interpolate({ { 0, cincoEpi } }, &distCoord) : 0.0; //proton.dist(cincoE);
	double t_1   = t_pion_PHsimple(cincoEpi, creator, tpf, distCoord, tpEmin, tpEmax);     //esto no es lossesPH porque son perdidas solo del canal de produccion de piones
	double omega = omegaPHsimple(cincoEpi, creator, tpf, distCoord, tpEmin, tpEmax);
	...
	if (t_1 > 0.0 && omega > 0.0)	{
        ...
        //emissivity = P2(5.0) * (t_1 * nChargedPion/(nChargedPion+nNeutralPion)) * protonDist;
	}
	else	{
		emissivity = 0;
	}
...
double pgammaPionInj2(double E, const Particle& creator,	const ParamSpaceValues& tpf, 
						const SpaceCoord& distCoord, double tpEmin, double tpEmax)
{
	double cincoE = 5.0*E;
	...
	if (t_1 > 0.0 && omega > 0.0)	{
		...
		emissivity = P2(5.0)*nChargedPion*t_1*protonDist;
	}
	else	{
		emissivity = 0;
	}
