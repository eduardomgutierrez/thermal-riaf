...
double integSimpsonVec(Vector y, double dx)
{
	double sum_even = 0.0;
    double sum_odd = 0.0;
    size_t n = y.size();
    if (n % 2 == 0) {
        ...
        sum_odd += y[2*m-1];
        return dx * ( (y[0] + y[n] + 2.*sum_even + 4.*sum_odd)/3. + 0.5*(y[n]+y[n+1]) );
    } else {
        n = n-1;
        size_t m = n / 2;
        for (size_t j=1; j <= m-1; j++) {
        ...
        sum_odd += y[2*m-1];
        return dx/3. * (y[0] + y[n] + 2.*sum_even + 4.*sum_odd);
    }
}

double integSimpsonVecLog(Vector y, Vector x)
{
	double sum_even = 0.0;
    double sum_odd = 0.0;
    double dlogx = log(x[1]/x[0]);
    size_t n = y.size();
    if (n % 2 == 0) {
        n = n-2;
        size_t m = n / 2;
        for (size_t j=1; j <= m-1; j++) {
            sum_even += y[2*j]*x[2*j];
            sum_odd += y[2*j-1]*x[2*j-1];
        }
        sum_odd += y[2*m-1]*x[2*m-1];
        return dlogx * ( (y[0]*x[0] + y[n]*x[n] + 2.*sum_even + 4.*sum_odd)/3. + 0.5*(y[n]*x[n]+y[n+1]*x[n+1]) );
    } else {
        n = n-1;
        size_t m = n / 2;
        for (size_t j=1; j <= m-1; j++) {
        ...
        sum_odd += y[2*m-1]*x[2*m-1];
        return dlogx/3. * (y[0]*x[0] + y[n]*x[n] + 2.*sum_even + 4.*sum_odd);
    }
}
