...
double crossSectionBetheHeitler(double t)
{
	...
	if (pairThresholdPH/electronRestEnergy <= x && x <= aux2) {
...
double crossSectionBetheHeitler2(double t)
{
	double aux2	= 4.0;
    double xth = pairThresholdPH / electronRestEnergy;
	double x = t / electronRestEnergy;

	if (xth <= x && x <= aux2) {
		return 1.2e-27 * P3((x-xth)/x);  //cm2
	} else if ( x > aux2) {
        double xx = 2*x;
        double factor2 = 0.5490 + 2.7101*log(xx) - P2(log(xx)) + 0.6667 * P3(log(xx));
        double factor = 3.1111*log(xx) - 8.0741 + P2(2./x) * factor2;
		return 5.8e-28 * factor;
	} else {
		return 0.0;
	}
}
...
{	
	...
	if (pairThresholdPH/electronRestEnergy < x && x <= aux3)	{
...
double crossSectionHadronic(double E)
{
    ...
    return E > pionThresholdH ? 1e-27 * (34.3 + 1.88*l*(1. + 0.25*l))*P2(1.-P2(P2(pionThresholdH/E))) : 0.;
}
