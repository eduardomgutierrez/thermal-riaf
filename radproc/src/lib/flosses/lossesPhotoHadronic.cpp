...
double lossesPhotoPair_2(double E, Particle& p, const ParamSpaceValues& tpf, 
							const SpaceCoord& psc, double phEmin, double phEmax)
{
	double gammap = E/(p.mass*cLight2);
	double cte	=	0.5*cLight/P2(gammap);
	double a2  = pairThresholdPH/(2*gammap);
 
	double a_pa = std::max(a2, phEmin);
    
    double integ = integSimpsonLog(a_pa, phEmax, [gammap, &p, &tpf, &psc] (double Eph)
		{
